...
describe("parseIntOrThrow", () => {
  test("It throws an error if the value provided isn't a string", () => {
    ...
    expect(() => parseIntOrThrow(value)).toThrow(
      new Error(`Expected value to be a string, got ${typeof value}`),
    );
  });
  ...
  test("It throws an error if the value provided isn't a valid number string", () => {
    const value = "invalid";
    expect(() => parseIntOrThrow(value)).toThrow(
      new Error(`Provided value was not a valid number string ${value}`),
    );
  });
